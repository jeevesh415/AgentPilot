# Creating a plugin
An **AgentPilot plugin** is a collection of modules bundled together and placed in the `src/plugins` directory. By organizing related modules into a single plugin, you can keep all modules related to a feature or integration together, making development and maintenance easier.
...
1. **Create a directory for your plugin under `src/plugins`**  
   For example, if your plugin is called `my_plugin`, create the following structure:
   ```
   src/
     plugins/
       my_plugin/
         ...
         widgets/
           __init__.py
           my_widget_module.py
         # (other module type folders as needed, e.g. environments, members, etc.)
         # (optional: src/ for additional code)
   ```
...
3. **(Optional) Add extra source code**  
   If your plugin needs additional code, you can add a `src` folder inside your plugin directory.

4. **Ensure each folder has an `__init__.py` file**  
   This makes sure Python recognizes them as packages.

## How Plugins Are Loaded

AgentPilot automatically discovers and loads plugins from the `src/plugins` directory at startup. Each module inside a plugin is registered and made available in the app, just like core modules.
